...
class NotActiveUsers extends StatefulWidget {
  @override
  _NotActiveUsersState createState() => _NotActiveUsersState();
}

class _NotActiveUsersState extends State<NotActiveUsers> {
  ...
  userList() {
    return Container(
        child: ListView.builder(
            ...
            itemCount: adminController.notActiveUsersList.length,
            itemBuilder: (_, int index) {
              return GestureDetector(
                onTap: () {
                  Get.to(ShowUser(
                    userModel: adminController.notActiveUsersList[index],
                  ));
                  //  onTapList(index: index,userModel: adminController.notActiveUsersList[index]);
                },
                child: Padding(
                            ...
                            children: [
                              FadeInImage.assetNetwork(
                                ...
                                image: adminController
                                    .notActiveUsersList[index].userImg ?? "",
                                fit: BoxFit.cover,
                              ),
                              SizedBox(width: 20.h),
                              Text(
                                  adminController
                                      .notActiveUsersList[index].fullName ?? "",
                                  style: black15Style()),
                            ],
                          ),
                        ],
